...
export async function GET(request: NextRequest) {
  ...
  // Get params
  const investment = parseInt(searchParams.get("p") || "100000");
  const expectedReturn = parseFloat(searchParams.get("r") || "12");
  const timePeriod = parseInt(searchParams.get("y") || "10");
  const totalValue = parseInt(searchParams.get("total") || "0");
  ...
  const returnsPercent = invested > 0 ? Math.round((returns / invested) * 100) : 0;
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          <span style={{ fontSize: "18px", color: "#6B7280" }}>
            Calculate your lumpsum returns at dhankit.com
          </span>
        </div>
      </div>
    ),
