...
export async function GET(request: NextRequest) {
  ...
  // Get params
  const loanAmount = parseInt(searchParams.get("p") || "5000000");
  const interestRate = parseFloat(searchParams.get("r") || "8.5");
  const tenureYears = parseInt(searchParams.get("y") || "20");
  const monthlyEMI = parseInt(searchParams.get("emi") || "0");
  const totalAmount = parseInt(searchParams.get("total") || "0");
  const totalInterest = parseInt(searchParams.get("interest") || "0");

  // Calculate interest percentage of total
  const interestPercent = loanAmount > 0 ? Math.round((totalInterest / loanAmount) * 100) : 0;
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          <span style={{ fontSize: "18px", color: "#6B7280" }}>
            Calculate your loan EMI at dhankit.com
          </span>
        </div>
      </div>
    ),
