...
export async function GET(request: NextRequest) {
  ...
  // Get params
  const principal = parseInt(searchParams.get("p") || "100000");
  ...
  const useSimpleInterest = searchParams.get("si") === "1";

  // Calculate percentage of returns
  const returnsPercent = invested > 0 ? Math.round((interest / invested) * 100) : 0;

  const interestTypeLabel = useSimpleInterest
    ? "Simple Interest"
    : `${getCompoundingLabel(compounding)} Compounding`;
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          <span style={{ fontSize: "18px", color: "#6B7280" }}>
            Calculate your FD returns at dhankit.com
          </span>
        </div>
      </div>
    ),
